...
export const Header = () => {
  const {user} = useUser() || {}
  return (
    <header className={`d-flex justify-content-between align-items-center py-4 ${styles.header}`}>
      ...
      {user? <p>Hola {user}</p> : ''}</div>
        <IconShoppingCart className={styles.icon} width={50} height={50}></IconShoppingCart></div>
    </header>
  )
}
